...
**C5 — Temporal-Leakage Artifact on Public Benchmarks**. We show that the FinBen/PIXIU stock-movement suite (ACL18, BigData22, CIKM18) measures memorization rather than forecasting when evaluated with a modern LLM (Section 5.9). Across all three benchmarks gemini-3.5-flash reaches 61–79% directional accuracy with the benchmark context, but **as high or higher (72–80%) with no context at all** — given only the ticker and target date — and famous mega-cap tickers are predicted far more accurately than obscure ones (up to 95% on CIKM18). Since the prompts do not leak (price context ends before the target date), the signal is **temporal leakage** through pre-training on the 2014–2021 windows. A cross-market causal control confirms it: the same no-context protocol on Chinese A-shares — the model's barely-memorized 2025 deployment market — scores 48.6% (chance), not 72–80%. Any "LLM beats baseline" result on a benchmark pre-dating the model's training cutoff is confounded; leakage-resistance (e.g., our 2025 walk-forward sample) is a prerequisite for valid LLM-finance evaluation.
