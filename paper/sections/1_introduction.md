...
**C5 — Temporal-Leakage Artifact on Public Benchmarks**. We show that the canonical public stock-movement benchmark ACL18 (used by FinBen/PIXIU to score financial LLMs) measures memorization rather than forecasting when evaluated with a modern LLM (Section 5.9). gemini-3.5-flash reaches 67.2% directional accuracy with the benchmark context, but **73.3% with no context at all** — given only the ticker and target date — and famous mega-cap tickers are predicted more accurately than obscure ones. Since the prompt does not leak (price context ends before the target date), the signal is **temporal leakage** through pre-training on the 2014–2016 window. Any "LLM beats baseline" result on a benchmark pre-dating the model's training cutoff is confounded; leakage-resistance (e.g., our 2025 walk-forward sample) is a prerequisite for valid LLM-finance evaluation.
