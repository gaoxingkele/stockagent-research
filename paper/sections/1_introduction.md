...
**C3 — Stratified-Sampling Artifact**. We empirically demonstrate that stratified evaluation overstates the contribution of LLM-based hybrid signals. On a 1,000-anchor stratified sample (25% expert-pattern-positive), our best hybrid router achieves +54% Top-10% return over a pure-LGBM baseline; on a 6,000-anchor walk-forward random sample (8% positive rate, matching reality), the same router achieves -15% — a sign reversal of 69 percentage points (Section 5.5). This finding constitutes a methodological warning: many recent "LLM-augmented" financial benchmarks may report inflated effect sizes due to undisclosed stratification.
