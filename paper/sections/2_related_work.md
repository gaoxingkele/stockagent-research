...
**Architectural limits of LLMs as alpha generators.** Two recent results give a mechanistic basis for *why* our identified LLM contribution is null (Section 5.10) rather than merely small. First, *proactive interference*: Wang and Sun [wang2025proactive] show that LLM retrieval of the latest value in a stream of semantically related updates decays log-linearly toward zero as prior updates accumulate — a working-memory bottleneck independent of (and unfixable by enlarging) the context window, and not remedied by prompt engineering, observed across 30+ models. This directly undercuts feeding long multi-period feature histories or order-flow streams to an LLM and expecting it to track the *current* market state. Second, *mode collapse*: Jiang et al. [jiang2025hivemind] (NeurIPS 2025 Best Paper) document an "Artificial Hivemind" across 70+ models — open-ended generations converge to high-probability responses both within a model and across model families, so neither higher sampling temperature nor multi-model ensembling restores diversity. Because exploitable alpha lives in the long tail of the financial-knowledge distribution, a generator biased toward the mode is structurally positioned to reproduce already-arbitraged "traditional wisdom." These limits frame our contributions C5 (the identified LLM contribution to onset detection is ≈0, and LLM-as-weak-supervisor is harmful) and C6 (the regime-conditional information that does exist is sub-cost) not as artifacts of our setup but as expected consequences of using an LLM as an ideator/predictor rather than as a rigorous execution-and-falsification engine under human-supplied hypotheses.
