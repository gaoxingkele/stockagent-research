...
**Table 4: directional accuracy across the FinBen stock-movement suite**

| Benchmark (period) | n | Baseline | LLM full ctx | **LLM no ctx** | ctx Δ | mega-cap | other |
|---|---|---|---|---|---|---|---|
| ACL18 (2014–16) | 3,720 | 49.1% | 67.2% | **73.3%** | −6.05pp | 74.0% | 66.5% |
| BigData22 (2019–21) | 1,472 | 55.2% | 60.6% | **71.9%** | −11.22pp | 73.5% | 58.9% |
| CIKM18 (2017) | 1,143 | 53.4% | 78.8% | **80.3%** | −1.47pp | 95.4% | 75.7% |

The same signature appears on **all three** benchmarks (Figure `e3_leakage_suite`). **First**, LLM accuracy (60–79%) is implausible for genuine next-day prediction — published results on these datasets cluster at 52–58%, and the ceiling for honest next-day forecasting is near 53% (the tabular baseline sits at chance). **Second, and decisively, stripping the entire input — all prices and tweets — *raises* accuracy on every benchmark** (no-context ≥ full-context throughout; context contributes −1.5 to −11.2pp). A model that predicts better with no data than with the benchmark's data is not analysing; it is recalling. **Third**, this is corroborated by a uniform mega-cap effect: famous tickers (AAPL/AMZN/MSFT/CSCO/…) are predicted far more accurately than obscure ones (up to 95.4% on CIKM18) — exactly the gradient expected if the signal is memorized historical price movement. The prompts do not leak (price context always ends 1–4 trading days before the target date); the leakage is **temporal**, through the LLM's pre-training corpus, which post-dates all three 2014–2021 benchmark windows.

**Cross-market causal control.** All three benchmarks pre-date the model's training cutoff, so a temporal pre/post split is impossible. Instead we run the *identical* no-context protocol on Chinese A-shares — our 2025 deployment market, which the model has scarcely memorized. On 1,000 A-share anchors the no-context directional accuracy is **48.6% (95% date-clustered CI [45.4%, 51.7%])** — chance, against a 50.3% base rate — versus 72–80% on the US benchmarks. Same model, same protocol, opposite outcome: the US "skill" is provably recall of memorized US price history, not forecasting, and the A-share data is leakage-resistant. This is the causal control that turns the temporal-leakage diagnosis from suggestive into established.

The implication is a second, independent methodological warning: **evaluating a modern LLM on a stock-movement benchmark that pre-dates its training cutoff measures memorization, not forecasting, and any "LLM beats baseline" claim on such data is confounded by temporal leakage.** This is precisely why the contributions of this paper (C1–C4) are measured on a **2025** walk-forward sample that post-dates common training cutoffs: leakage-resistance is a prerequisite for a meaningful LLM-finance benchmark, not an optional nicety.

## 5.10 Leakage-Free Identification of the LLM's Contribution (C5)

The leakage-freeness established above is not merely hygiene: it is an **identification condition**. On contaminated data the LLM's prediction confounds reasoning with memorized recall, so the *contribution of LLM reasoning is unidentified* — every "LLM helps" estimate mixes the two. On a substrate where the no-context probe sits at chance (A-shares, §5.9), the memory channel is null, so any incremental signal the LLM adds over a tabular baseline is identified as reasoning. We exploit this with three measurements (date-clustered CIs throughout; leakage validity certified by the §5.9 A-share probe). Figure `identify_summary` summarises.

**(i) Identified LLM contribution ≈ 0.** On the 1,000 A-share anchors, the incremental contribution of the LLM signal over the LGBM Pattern Core — the partial rank correlation with the realised forward return, controlling for the baseline — is **+0.033 (95% CI [−0.037, +0.110])** for the raw LLM and **+0.006 ([−0.062, +0.078])** for the expert-prompted LLM. Both span zero. This is, to our knowledge, the first *identified* (not memory-confounded) estimate of LLM value-add in stock-movement onset prediction, and it is statistically indistinguishable from zero.

**(ii) LLM-as-weak-supervisor is harmful.** Using the LLM as a knowledge injector rather than a predictor — turning its signals into labeling functions and refining the training labels of a downstream learner — *lowers* held-out performance: arm A (true labels) reaches RankIC +0.109 [+0.037, +0.184], while the LLM-weak-refined arm B reaches −0.040 [−0.114, +0.028], an identified change of **−0.149 (CI [−0.298, −0.009])**. On leakage-free data this degradation is cleanly attributable to the LLM's "knowledge", not to memorized labels.

**(iii) A de-biasing estimator that corrects contaminated benchmarks.** Because context *helps* on clean data but *hurts* on memorized data, the clean market calibrates a correction: the recall-corrected accuracy is `full − (no_context − chance)`, valid when the clean market's no-context sits at chance (it does, 0.486). Applied to FinBen, the headline 67–80% collapses to reasoning-only **ACL18 0.440, BigData22 0.388, CIKM18 0.485** — at or below chance — with a measured clean-market reasoning effect of only +0.02. The de-biasing estimator is the one component here that exports value to the wider field: it puts a number on how much of any contaminated-benchmark "skill" survives once memorization is removed.

Taken together, on the only leakage-free substrate available, LLM reasoning adds ≈nothing to A-share onset detection (and hurts as weak supervision), while the field's positive US-benchmark numbers are almost entirely memorization. We present this as identification/measurement, not a performance result; the clean null is the point. Concurrent work [ktdfin2026, profitmirage2025] separates memory from reasoning by masking and quantifies agent leakage; our distinct elements are the explicit identification condition and the cross-market de-biasing estimator.
