...
### SIGN-V1: Externalize state -- never let the LLM track evolving market state in-context
**Trigger:** any design where an LLM is fed long time-series / multi-period feature history and asked to infer the CURRENT regime, crowding, or position.
**Instruction:** Proactive interference (Wang & Sun 2025, arXiv:2506.08184) makes LLM retrieval of the LATEST value decay log-linearly as prior updates accumulate; prompt engineering ("forget history", "use only the latest") does NOT fix it and can worsen it; bigger context windows worsen it. So the SYSTEM must maintain state externally and pass the LLM only a single-period, static, already-computed latest feature vector, clearing history each call. All our pipelines already do this (point-in-time features, parquet state, no in-context time-series memory) -- keep it that way.
**Reason:** A model that confuses "patient's current blood pressure" under accumulating updates will collapse on noisy multi-state market data; externalized point-in-time state sidesteps the architectural bottleneck entirely.

### SIGN-V2: LLM ideation mode-collapses -- humans supply the edge hypothesis, AI executes
**Trigger:** asking an LLM to "find a factor / propose a strategy / generate diverse alpha".
**Instruction:** Mode collapse (Jiang et al. 2025, Artificial Hivemind, NeurIPS Best Paper, arXiv:2510.22954) makes LLM open-ended output converge to the highest-probability "traditional wisdom" (RSI, cointegration pairs, generic CV) both within a model and ACROSS model families (so multi-model ensembling does NOT add idea diversity; raising temperature does not either). Alpha lives in the long tail this mechanism suppresses. Therefore the edge HYPOTHESIS ("who pays you and why") must come from a human; the AI's role is rigorous execution + adversarial falsification (estimators, cost models, permutation nulls, walk-forward, DSR). Our onset-motif (C6) came exactly this way -- human analogy (promoter/transcription-factor) + AI-built information-theoretic probe -- not from "LLM, find alpha".
**Reason:** Treating the LLM as an ideator manufactures homogeneous, crowded, already-arbitraged strategies; treating it as a tireless skeptic/auditor is where it adds real value.

### SIGN-V3: Audit AI-generated research code for prior-noise leakage + optimizer/boundary errors
**Trigger:** AI-written factor code, RAG-grounded generation, or portfolio-optimizer code.
**Instruction:** (a) Even under strict RAG, the model leaks pretrained "traditional indicator" priors -- static-lint AI-generated factor code for unsanctioned generic-TA library calls. (b) Optimizer code (mean-variance / max-Sharpe QP) defaults to textbook forms and mishandles PSD covariance, no-short / leverage bounds, and cost terms -- run boundary tests (extreme covariance -> singular?, constraints honored?, slippage/stamp/impact present?). These are the When-Alpha-Breaks / Brave-New-Backtest failure modes operationalized.
**Reason:** Beautiful-but-wrong AI scaffolding is now near-zero-cost to produce; the cost has moved to auditing it. Cheap lints + boundary tests catch the silent errors that would otherwise ship as "alpha".
