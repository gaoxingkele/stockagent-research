...
FACTOR_LAB = PROD_ROOT / "output/factor_lab_3y/factor_groups_extension"
...
def load_factors() -> pd.DataFrame:
    ...
    files = sorted(FACTOR_LAB.glob("*_ext*.parquet"))
    if not files:
        logger.warning(f"No factor files in {FACTOR_LAB}, returning empty")
        return pd.DataFrame()
    parts = [pd.read_parquet(f) for f in files]
